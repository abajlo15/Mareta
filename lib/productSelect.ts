...
export const FEATURED_PRODUCTS_SELECT =
  "position, product:products(*, product_sizes(size, stock), subcollection:subcollections!products_subcollection_id_fkey(id, name, thumbnail_url, collection_id), product_collections(collection:collections(id, name, slug, thumbnail_url)))" as const;

export const ADMIN_PRODUCTS_SELECT_WITH_POSITIONS =
  "id, name, description, price, discount_percentage, categories, subcollection_id, stock, is_shirt, is_polarized, images, product_sizes(size, stock), subcollection:subcollections!products_subcollection_id_fkey(name, thumbnail_url, collection_id), product_collections(position, collection:collections(id, name, slug, thumbnail_url)), subcollection_product_positions!subcollection_product_positions_product_id_fkey(position)" as const;

export const ADMIN_PRODUCTS_SELECT_BASE =
  "id, name, description, price, discount_percentage, categories, subcollection_id, stock, is_shirt, is_polarized, images, product_sizes(size, stock), subcollection:subcollections!products_subcollection_id_fkey(name, thumbnail_url, collection_id), product_collections(position, collection:collections(id, name, slug, thumbnail_url))" as const;

export const ADMIN_PRODUCTS_SELECT_NO_POSITION =
  "id, name, description, price, discount_percentage, categories, subcollection_id, stock, is_shirt, is_polarized, images, product_sizes(size, stock), subcollection:subcollections!products_subcollection_id_fkey(name, thumbnail_url, collection_id), product_collections(collection:collections(id, name, slug, thumbnail_url))" as const;
